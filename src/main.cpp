 ...
 */
        PushMatrix(model); // Guardamos matriz model atual na pilha
            model = model * Matrix_Translate(0.0f, 0.00f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com uma translação para a cabeça
            PushMatrix(model); // Guardamos matriz model atual na pilha
                model = model // Atualizamos matriz model (multiplicação à direita) com a rotação da cabeça
                      * Matrix_Rotate_Z(3.1415);  // TERCEIRO rotação Z de Euler
                ...
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    ...
                    PushMatrix(model); // Guardamos matriz model atual na pilha
                        model = model * Matrix_Scale(0.8f, 1.0f, 0.2f); // Atualizamos matriz model (multiplicação à direita) com um escalamento do braço direito
...
/*cabeça*/PushMatrix(model); // Guardamos matriz model atual na pilha
            model = model * Matrix_Translate(0.0f, 1.05f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com uma translação para a cabeça
            PushMatrix(model); // Guardamos matriz model atual na pilha
                model = model // Atualizamos matriz model (multiplicação à direita) com a rotação da cabeça
                      * Matrix_Rotate_Z(3.1415);  // TERCEIRO rotação Z de Euler
                      //* Matrix_Rotate_Y(g_AngleY)  // SEGUNDO rotação Y de Euler
                ...
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model // Atualizamos matriz model (multiplicação à direita) com a rotação do braço direito
                        ...
                        * Matrix_Rotate_X(g_AngleX); // PRIMEIRO rotação X de Euler
                    PushMatrix(model); // Guardamos matriz model atual na pilha
                        model = model * Matrix_Scale(0.4f, 0.4f, 0.2f); // Atualizamos matriz model (multiplicação à direita) com um escalamento do braço direito
                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                        DrawCube(render_as_black_uniform); // #### BRAÇO DIREITO // Desenhamos o braço direito
                    PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                PopMatrix(model);
...
/*R_arm*/PushMatrix(model); // Guardamos matriz model atual na pilha
            model = model * Matrix_Translate(-0.55f, 1.0f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com uma translação para o braço direito
            PushMatrix(model); // Guardamos matriz model atual na pilha
                model = model // Atualizamos matriz model (multiplicação à direita) com a rotação do braço direito
                      * Matrix_Rotate_Z(g_AngleZ)  // TERCEIRO rotação Z de Euler
                      * Matrix_Rotate_Y(g_AngleY)  // SEGUNDO rotação Y de Euler
                ...
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    ...
                    PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
            /*Mão*/ PushMatrix(model); // Guardamos matriz model atual na pilha
                        model = model * Matrix_Translate(0.0f, -0.65f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com a translação do antebraço direito
                        //model = model // Atualizamos matriz model (multiplicação à direita) com a rotação do antebraço direito
                           // * Matrix_Rotate_Z(g_ForearmAngleZ)  // SEGUNDO rotação Z de Euler
                           // * Matrix_Rotate_X(g_ForearmAngleX); // PRIMEIRO rotação X de Euler
...
/*Perna direita*/
        PushMatrix(model); // Guardamos matriz model atual na pilha
            model = model * Matrix_Translate(-0.2f, -0.05f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com uma translação para o braço direito
            PushMatrix(model); // Guardamos matriz model atual na pilha
                //model = model // Atualizamos matriz model (multiplicação à direita) com a rotação do braço direito
                //      * Matrix_Rotate_Z(g_AngleZ)  // TERCEIRO rotação Z de Euler
                //      * Matrix_Rotate_Y(g_AngleY)  // SEGUNDO rotação Y de Euler
                ...
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    ...
                    PushMatrix(model); // Guardamos matriz model atual na pilha
                        ...
                        DrawCube(render_as_black_uniform); // #### ANTEBRAÇO DIREITO // Desenhamos o antebraço direito
                    PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
            /*pé*/  PushMatrix(model); // Guardamos matriz model atual na pilha
                        model = model * Matrix_Translate(0.0f, -0.65f, 0.125f); // Atualizamos matriz model (multiplicação à direita) com a translação do antebraço direito
                        //model = model // Atualizamos matriz model (multiplicação à direita) com a rotação do antebraço direito
                           // * Matrix_Rotate_Z(g_ForearmAngleZ)  // SEGUNDO rotação Z de Euler
                           // * Matrix_Rotate_X(g_ForearmAngleX); // PRIMEIRO rotação X de Euler
                        PushMatrix(model); // Guardamos matriz model atual na pilha
